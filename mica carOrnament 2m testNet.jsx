    ...
    for (var i = stt; i <= arrcut.length - 1; i++) { // loop làm file in
        ...
        if ((yPosition + app.activeDocument.height + 30 + hLast > hallcut) && (xPosition + app.activeDocument.width + 30 + wLast) > wallcut) {
            app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);
            {
                ...
                // for (var g = 0; g < app.activeDocument.layerSets["SPOT"].artLayers.length; g++) {
                //     app.activeDocument.activeLayer = app.activeDocument.layerSets["SPOT"].artLayers[g];
                //     app.doAction("getSelectionLayer", "go");
                //     app.activeDocument.activeLayer = app.activeDocument.layerSets["CMYK"].artLayers[g];
                //     app.doAction("movecenter 2side", "go");
                //     app.activeDocument.activeLayer = app.activeDocument.layerSets["SPOT2"].artLayers[g];
                //     app.doAction("movecenter 2side", "go");
        ...
        }
        {
            ...
            }
            {
                ...
                { // crop xóa khoảng trắng và crop cỡ 9cm
                    app.open(File(FileDesign + "/" + arrcut[i].sku + " front.png"));
                    {
                        ...
                        desc40.putReference(idnull, ref17);
                        var idFrom = charIDToTypeID("From");
                        var ref18 = new ActionReference();
                        var idcsel = charIDToTypeID("csel");
                        var idfsel = charIDToTypeID("fsel");
                        ref18.putProperty(idcsel, idfsel);
                        desc40.putReference(idFrom, ref18);
                        var idTlrn = charIDToTypeID("Tlrn");
                        var idPxl = charIDToTypeID("#Pxl");
                        desc40.putUnitDouble(idTlrn, idPxl, 1.500000);
                        executeAction(idMk, desc40, DialogModes.NO);
                        app.activeDocument.pathItems[0].makeSelection();
                        var PSpotKhung = app.activeDocument.selection.bounds;
                        app.activeDocument.crop(PSpotKhung, 0, PSpotKhung[2] - PSpotKhung[0], PSpotKhung[3] - PSpotKhung[1]);
                    ...
                    app.activeDocument.activeLayer.translate(xPosition, (yPosition) * (-1));
                    {
                        ...
                        saveImageTool(arrcut[i], "front");
                        app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);
                    }
                }
                { // crop xóa khoảng trắng và crop cỡ 9cm
                    try {
                        app.open(File(FileDesign + "/" + arrcut[i].sku + " back.png"));
                    } catch (error) {
                        app.open(File(FileDesign + "/" + arrcut[i].sku + " front.png"));
                        // app.doAction("canvasHoriz", "go");
                    }
                    ...
                    if (app.activeDocument.width > app.activeDocument.height) app.activeDocument.rotateCanvas(90);
                    app.activeDocument.activeLayer.name = arrcut[i].stt;
                    app.activeDocument.activeLayer.duplicate(app.documents["GLLM"].layerSets["SPOT2"], ElementPlacement.PLACEATBEGINNING);
                    app.activeDocument.close(SaveOptions.DONOTSAVECHANGES);

                    app.activeDocument.activeLayer = app.activeDocument.layerSets["SPOT2"].artLayers.getByName(arrcut[i].stt);
                    app.doAction("moveZero", "autoUv");
                    app.activeDocument.activeLayer.translate(xPosition, (yPosition) * (-1));
